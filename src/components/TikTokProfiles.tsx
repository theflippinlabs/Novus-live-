...
const TEXT = {
  en: {
    ...
    groups: "Groups",
    groupsHint: "Sort your streamers into groups (e.g. agency, friends, VIP). The top bar then shows one group at a time.",
    newGroup: "New group name",
    create: "Create",
    rename: "Rename",
    renamePrompt: "New name for this group:",
    deleteGroup: "Delete group",
    confirmDelete: (g: string) => `Delete the group "${g}"? Its accounts stay followed, just without a group.`,
    noGroup: "No group",
  ...
  fr: {
    ...
    groups: "Groupes",
    groupsHint: "Range tes livers dans des groupes (ex. agence, potes, VIP). La barre du haut affiche alors un groupe à la fois.",
    newGroup: "Nom du nouveau groupe",
    create: "Créer",
    rename: "Renommer",
    renamePrompt: "Nouveau nom du groupe :",
    deleteGroup: "Supprimer le groupe",
    confirmDelete: (g: string) => `Supprimer le groupe « ${g} » ? Ses comptes restent suivis, simplement sans groupe.`,
    noGroup: "Sans groupe",
...
export function TikTokProfiles() {
  ...
  const byGroup = groups.map((g) => ({ g, list: profiles.filter((p) => g.members.includes(p.toLowerCase())) }));
  ...
  return (
    <>
      ...
      {byGroup.map(({ g, list }) => (
        <div key={g.id} className="group-block">
          <div className="group-head">
            ...
            <button className="btn sm ghost" onClick={() => renameGroup(g)} disabled={busy} title={tx.rename} aria-label={`${tx.rename} ${g.name}`}>
              ✎
            </button>
            <button className="btn sm ghost" onClick={() => deleteGroup(g)} disabled={busy} title={tx.deleteGroup} aria-label={`${tx.deleteGroup} ${g.name}`}>
              ✕
            </button>
          </div>
          {list.length ? list.map(row) : <div className="small muted" style={{ padding: "4px 0 8px" }}>{tx.emptyGroup}</div>}
        </div>
      ))}
      ...
      <div className="row">
        ...
        />
        <button className="btn" onClick={createGroup} disabled={busy || !groupDraft.trim() || groups.length >= 30}>
          + {tx.create}
        </button>
      </div>
      <div className="small muted" style={{ marginTop: 6 }}>
